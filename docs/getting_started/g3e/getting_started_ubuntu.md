...
# RZ/G3E Ubuntu Bootable Tool

[Back to Getting Started >](../../getting_started/index.md#step-4-create-a-bootable-microsd-card){ .md-button .btn-right }

## 1. Overview

The **RZ/G3E Ubuntu Bootable Tool** is a Bash script designed to streamline the creation and customization of bootable ARM64 Linux disk images for Ubuntu 24.04 on the RZ/G3E platform. It offers a user-friendly, menu-based interface using `whiptail` and `zenity`, making it easy to create, and apply custom configurations. The tool also supports writing images directly to an external media (e.g., SD card).
...
- Create your working directory, and decompress packages:
    ...
    mkdir -p ${WORK_DIR}
    cd ${DL_DIR}
    unzip RTK0EF0045Z0043AZJ-v*.zip
    tar xf RTK0EF0045Z0043AZJ-v*/rz-debian-support-v*.tar.gz -C ${WORK_DIR}
    unzip RTK0EF0045Z0047AZJ-v*_EN.zip
    tar xf RTK0EF0045Z0047AZJ-v*_EN/rz-graphics-v*.tar.gz -C ${WORK_DIR}
    unzip RTK0EF0045Z0045AZJ-v*_EN.zip
    tar xf RTK0EF0045Z0045AZJ-v*_EN/rz-codecs-v*.tar.gz -C ${WORK_DIR}
    ```
    {: .dollar }
...
- Execute `#!bash ./make_bootable_tool.sh` to run the GUI installation tool:
...
- For quick fix, find a valid URL for `ubuntu-24.04.4-live-server-arm64.iso` from the [Ubuntu old releases](https://old-releases.ubuntu.com/releases/noble/) or other trusted sources, then [update the value of ISO_URLS accordingly](#4251-iso_urls).
